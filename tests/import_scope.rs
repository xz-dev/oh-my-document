...
fn w(root: &PathBuf, rel: &str, c: &str) {
    let f = root.join(rel);
...
fn recursive_includes_subdirs() {
    let r = tmp("rec");
    w(&r, "a.md", "1"); w(&r, "sub/b.md", "2"); w(&r, "sub/deep/c.md", "3");
    let s = resolve(&r, &[]);
...
fn broken_symlink_is_a_problem_not_full_coverage() {
    ...
    let s = resolve(&r, &[]);
    assert!(s.problems.iter().any(|p| p.contains("broken")), "{:?}", s.problems);
}
...
fn symlink_cycle_reported() {
    ...
    let s = resolve(&r, &[]);
    assert!(s.problems.iter().any(|p| p.contains("cycle")), "{:?}", s.problems);
}

#[test]
fn dot_omd_is_importable_not_hidden() {
    let r = tmp("omddot");
    w(&r, ".omd/state.toml", "x"); w(&r, ".omd/commits/c.toml", "y"); w(&r, "normal.md", "1");
    let s = resolve(&r, &[]);
    assert!(has(&s, ".omd/state.toml"), ".omd/ must be importable, not hidden");
}

#[test]
fn dir_pattern_excludes_subtree() {
    let r = tmp("dirpat");
    w(&r, "keep/a.md", "1"); w(&r, "vendor/lib.rs", "2");
