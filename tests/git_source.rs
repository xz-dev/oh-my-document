...
fn exact_commit_blob_read() {
    let (r, c1) = make_repo();
    let g = parse_git_ref(&format!(
        "git::{{\"repo\":\"{}\",\"commit\":\"{}\",\"path\":\"f.txt\"}}",
        r.display(),
        c1
    ))
    .unwrap();
...
fn head_movement_does_not_change_observation() {
    ...
    git(&r, &["add", "."]);
    git(&r, &["-c", "commit.gpgsign=false", "commit", "-qm", "c2"]);
...
fn floating_ref_name_rejected() {
    ...
    let res = parse_git_ref(&format!(
        "git::{{\"repo\":\"{}\",\"commit\":\"HEAD\",\"path\":\"f.txt\"}}",
        r.display()
    ));
...
fn in_commit_symlink_resolves_within_commit() {
    ...
    let blob = read_blob(&g).unwrap();
    assert_eq!(
        blob, b"historical content X",
        "symlink must resolve in-commit"
    );
    let _ = std::fs::remove_dir_all(&r);
}
