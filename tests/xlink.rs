...
fn cross_store_link_queryable_both_ends() {
    ...
    b.run(&["init", "impl.py"]);
    b.run(&[
        "commit", "commit", "impl.py", "--range", "0-16", "--reason", "py-range",
    ]);
    let bsid = b.store_id();
    ...
    let (c, o, e) = a.run(&[
        ...
        "0-5",
        "--xlink-to",
        &format!("peer:{bsid}:impl.py@text:0-16"),
        "--reason",
        "xlang",
    ]);
    ...
    let ast = a.state();
    assert!(
        ast.contains(&format!("peer:{bsid}:range:impl.py@text:0-16")),
        "A records peer-qualified target: {ast}"
    );
    // B's state records the inbound credential protecting its py range.
    let bst = b.state();
    assert!(
        bst.contains("range:impl.py@text:0-16") && bst.contains("inbound"),
...
fn cross_store_link_no_metadata_merge() {
    ...
    b.run(&["init", "b.md"]);
    b.run(&[
        "commit", "commit", "b.md", "--range", "0-4", "--reason", "r",
    ]);
    let bsid = b.store_id();
    a.run(&["register", &bsid, &b.0.join(".omd").to_string_lossy()]);
    a.run(&[
        "commit", "commit", "a.md", "--range", "0-4", "--reason", "r",
    ]);
    a.run(&[
        "commit",
        "commit",
        ...
        "0-2",
        "--xlink-to",
        &format!("peer:{bsid}:b.md@text:0-4"),
        "--reason",
        "x",
    ]);
    ...
    assert!(
        shared.is_empty(),
        "no shared commit ids across stores: {shared:?}"
    );
...
fn gc_reports_offline_consumer_reason_named() {
    ...
    assert!(
        o.contains("protected") || o.contains("collected") || o.contains("released"),
        "gc reports protection detail: {o}"
    );
    let st = b.state();
    assert!(
        st.contains("store-A-offline"),
