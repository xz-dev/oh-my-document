...
impl Store {
    fn new(tag: &str) -> Self {
        let r = std::env::temp_dir().join(format!("omd-xs-{}-{}", tag,
...
impl Drop for Store { fn drop(&mut self) { let _ = std::fs::remove_dir_all(&self.0); } }
...
fn inbound_credential_persists_before_consumer_publishes() {
    ...
    let (c, out, _) = b.run(&["protect", "b1-target", "--peer", "store-A", "--record", "record-A1"]);
    assert_eq!(c, 0, "{out}");
    assert!(out.contains("credential"), "must return credential id");
    assert!(b.state().contains("record-A1"), "credential persisted in B's state");
}
...
fn credential_without_publish_keeps_target_protected() {
    ...
    b.run(&["init", "b.md"]);
    b.run(&["protect", "keep-target", "--peer", "store-A", "--record", "pending-r"]);
    // gc --content must NOT free the protected target's content.
    let (c, out, _) = b.run(&["gc", "--content"]);
    assert_eq!(c, 0, "{out}");
    assert!(out.contains("protected"), "gc must report protected targets");
}
...
fn copied_store_refuses_writes_until_activated() {
    ...
    let copy_root = std::env::temp_dir().join(format!("omd-copy-{}",
        std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
    ...
    let st = std::fs::read_to_string(&st_path).unwrap().replace("activated = true", "activated = false");
    std::fs::write(&st_path, st).unwrap();

    // A business write on the copy must refuse.
    let o = Command::new(omd()).arg("--meta").arg(&dst)
        .args(["commit", "commit", "f.md", "--reason", "r"])
        .current_dir(&copy_root).output().unwrap();
    assert!(!o.status.success(), "unactivated copy must refuse business write");

    // gc must refuse too.
    let o2 = Command::new(omd()).arg("--meta").arg(&dst)
        .args(["gc", "--content"]).current_dir(&copy_root).output().unwrap();
    assert!(!o2.status.success(), "unactivated copy must refuse gc");

    // After activate, writes are allowed again (new store_id).
    let o3 = Command::new(omd()).arg("--meta").arg(&dst)
        .args(["activate"]).current_dir(&copy_root).output().unwrap();
    assert!(o3.status.success(), "activate must succeed: {:?}", String::from_utf8_lossy(&o3.stderr));
