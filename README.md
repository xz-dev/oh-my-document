...
- `--difftastic` is a **one-shot** permission to shell out to the `difft` binary for this call only: it structurally compares old vs new full contents and moves ranges judged structure-unchanged out of `dirty` into a `cosmetic` bucket. The flag never carries into the next call, and reads/queries/reindex never trigger it. When the tool is missing or fails, the affected ranges conservatively stay in `dirty` — never silently cosmetic. `commit cosmetic <path>` re-classifies under the credential-pinned version and batch-continues the cosmetic ranges.
...
- Audits and notes are append-only commit chains (`audit:<init>` / `note:<init>`), never flat records: `omd audit add|show|list|pass|fail|pending` walks a seed's link graph (both directions reported separately), judges every edge L0–L2, and appends three-state conclusions as patches; wiki `audit:<commit-id>` references and Link endpoints pin specific commits. Only structural `broken` exits 1 — `fail` and `pending` are records, not crashes. `commit unclean --reason "audit:<id>"` opens a tracked repair loop that a fix commit closes by linking back to the pinned audit.
